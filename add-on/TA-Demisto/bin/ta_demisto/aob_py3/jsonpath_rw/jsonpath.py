...
class DatumInContext(object):
    ...
    def __eq__(self, other):
        return isinstance(
            other,
            DatumInContext) and other.value == self.value and other.path == self.path and self.context == other.context
    ...
    def find(self, datum):
        ...
        def match_recursively(datum):
            ...
            if isinstance(datum.value, list):
                recursive_matches = [
                    submatch for i in range(
                        0, len(
                            datum.value)) for submatch in match_recursively(
                        DatumInContext(
                            datum.value[i], context=datum, path=Index(i)))]

            elif isinstance(datum.value, dict):
                recursive_matches = [submatch for field in datum.value.keys() for submatch in match_recursively(
...
class Fields(JSONPath):
    ...
    def get_field_datum(self, datum, field):
        if field == auto_id_field:
            ...
            try:
                # Do NOT use `val.get(field)` since that confuses None as a value and None due to `get`
...
class Slice(JSONPath):
    ...
    def __eq__(self, other):
        return isinstance(
            other,
            Slice) and other.start == self.start and self.end == other.end and other.step == self.step
