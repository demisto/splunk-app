    ...
    function(Backbone, jquery, splunk_js_sdk) {
        ...
        var DemistoView = Backbone.View.extend({
            ...
            trigger_setup: function trigger_setup() {
                ...
                var zerotrust_token_url_input_element = jquery("input[name=BEYOND_CORP_API]");
                var zerotrust_token_key_input_element = jquery("input[name=BEYOND_CORP_KEY]");
                var zerotrust_token_secret_input_element = jquery("input[name=BEYOND_CORP_SECRET]");
                var exclusion_list_input_element = jquery("input[name=EXCLUSION_LIST]");
                ...
                var the_zerotrust_url = zerotrust_token_url_input_element.val();
                var the_zerotrust_key = zerotrust_token_key_input_element.val();
                var the_zerotrust_secret = zerotrust_token_secret_input_element.val();
                var the_exclusion_list = exclusion_list_input_element.val();
                ...
                var sanitized_the_zerotrust_url = this.sanitize_string(the_zerotrust_url);
                var sanitized_the_zerotrust_key = this.sanitize_string(the_zerotrust_key);
                var sanitized_the_zerotrust_secret = this.sanitize_string(the_zerotrust_secret);
                var sanitized_the_exclusion_list = this.sanitize_string(the_exclusion_list);
                ...
                var error_messages_to_display = this.validate_inputs(
                    ...
                    sanitized_the_zerotrust_url,
                    sanitized_the_zerotrust_key,
                    sanitized_the_zerotrust_secret,
                    sanitized_the_exclusion_list,
                );
            ...
            create_custom_configuration_file: async function create_custom_configuration_file(
                ...
                var properties_to_update_1 = {
                    ...
                    exclusion_list: sanitized_the_exclusion_list,
                    beyond_corp_api: sanitized_the_zerotrust_url,
                    beyond_corp_secret: sanitized_the_zerotrust_secret,
                    beyond_corp_key: sanitized_the_zerotrust_key
                };
            ...
            encrypt_sensitive_data: async function encrypt_sensitive_data(
                ...
                var sanitized_the_api_key_realm = "TA-Demisto" + "-demistotoken";
                var sanitized_the_proxy_password_realm = "TA-Demisto" + "-proxypassword";
                var sanitized_the_zerotrust_key_realm = "TA-Demisto" + "-zerotrustkey";
                var sanitized_the_zerotrust_secret_realm = "TA-Demisto" + "-zerotrustsecret";
                // var username = "admin";
                ...
                // ZEROTRUST KEY check and write
                var check_if_zerotrust_key_exists = this.does_storage_password_exist(
                    storage_passwords_accessor,
                    sanitized_the_zerotrust_key_realm,
                    sanitized_the_zerotrust_key
                );

                var does_zerotrust_key_storage_password_exist = check_if_zerotrust_key_exists[0];
                var zerotrust_storage_passwords_found = check_if_zerotrust_key_exists[1];

                // If previous passwords were found, clear them all out before moving on
                if (does_zerotrust_key_storage_password_exist) {
                    // console.log("proxy password exists")
                    var i;
                    for (i = 0; i < zerotrust_storage_passwords_found.length; i++) {
                        await this.delete_storage_password(
                            storage_passwords_accessor,
                            sanitized_the_zerotrust_key_realm,
                            zerotrust_storage_passwords_found[i]
                        );
                    }
                } // else {
                    // console.log("proxy password does not exist")
                ...
                // ZEROTRUST SECRET check and write
                var check_if_zerotrust_secret_exists = this.does_storage_password_exist(
                    storage_passwords_accessor,
                    sanitized_the_zerotrust_secret_realm,
                    sanitized_the_zerotrust_secret
                );

                var does_zerotrust_secret_storage_password_exist = check_if_zerotrust_secret_exists[0];
                var zerotrust_storage_passwords_found = check_if_zerotrust_secret_exists[1];

                // If previous passwords were found, clear them all out before moving on
                if (does_zerotrust_secret_storage_password_exist) {
                    // console.log("proxy password exists")
                    var i;
                    for (i = 0; i < zerotrust_storage_passwords_found.length; i++) {
                        await this.delete_storage_password(
                ...
                if (sanitized_the_zerotrust_key.length > 0) {
                    await this.create_storage_password_stanza(
                        storage_passwords_accessor,
                        sanitized_the_zerotrust_key_realm,
                        sanitized_the_zerotrust_key_realm.split("-").slice(-1),
                        sanitized_the_zerotrust_key,
                    );
                }
                if (sanitized_the_zerotrust_secret.length > 0) {
                    await this.create_storage_password_stanza(
                        storage_passwords_accessor,
                        sanitized_the_zerotrust_secret_realm,
                        sanitized_the_zerotrust_secret_realm.split("-").slice(-1),
                        sanitized_the_zerotrust_secret,
                    );
                }
            ...
            get_template: function get_template() {
                template_string =
                    ...
                    "            <div class='user_input'>" +
                    "                <div class='text'>" +
                    "                    <input type='text' name='AUTHKEY' placeholder='' value='To generate new API key, login to Demisto application, go to Settings-->Integrations-->API Keys(Get Your Key)'></input>" +
                    "                </div>" +
                    "            </div>" +
                    "            <div class='title'>" +
                    "                <div>" +
                    "                    <h3>Location to Certificate (only used in non-ZeroTrust configurations):</h3>" +
                    "                </div>" +
                    "            </div>" +
                    "            <div class='user_input'>" +
                    "                <div class='text'>" +
                    "                    <input type='text' name='SSL_CERT_LOC' placeholder='Enter the full path to the SSL Certificate in the Splunk server to if you are using Self Signed/Internal CA signed certificate'></input>" +
                    "                </div>" +
                    "            </div>" +
                    "            <div class='title'>" +
                    "                <div>" +
                    ...
                    "            <div class='user_input'>" +
                    "                <div class='text'>" +
                    "                    <input type='text' name='HTTPS_PROXY_ADDRESS' placeholder='Enter HTTPS Proxy address in the following format - https://[hostname]:[port]'></input>" +
                    "                </div>" +
                    "            </div>" +
                    "            <div class='title'>" +
                    "                <div>" +
                    ...
                    "            <div class='user_input'>" +
                    "                <div class='text'>" +
                    "                    <input type='text' name='HTTPS_PROXY_USERNAME' placeholder='Enter HTTPS Proxy Username here if applicable'></input>" +
                    "                </div>" +
                    "            </div>" +
                    "            <div class='title'>" +
                    "                <div>" +
                    ...
                    "            <div class='user_input'>" +
                    "                <div class='text'>" +
                    "                    <input type='text' name='HTTPS_PROXY_PASSWORD' placeholder='Enter HTTPS Proxy Password here if applicable'></input>" +
                    "                </div>" +
                    "            </div>" +
                    "            <div class='title'>" +
                    "                <div>" +
                    "                    <h3>(Optional) \"Send to all servers\" exclusion list :</h3>" +
                    "                </div>" +
                    "            </div>" +
                    "            <div class='user_input'>" +
                    "                <div class='text'>" +
                    "                    <input type='text' name='EXCLUSION_LIST' placeholder='This is a comma delimited list of host names \(https\:\/\/\[hostname\]\:\[port\]\). If a trigger action has Send to all Servers, an incident will not be created for anything in this list.'></input>" +
                    "                </div>" +
                    "            </div>" +
                    "            <div class='title'>" +
                    "                <div>" +
                    "                    <h3>(Optional) ZeroTrust Tenant API Token URL:</h3>" +
                    "                </div>" +
                    "            </div>" +
                    "            <div class='user_input'>" +
                    "                <div class='text'>" +
                    "                    <input type='text' name='BEYOND_CORP_API' placeholder=''></input>" +
                    "                </div>" +
                    "            </div>" +
                    "            <div class='title'>" +
                    "                <div>" +
                    "                    <h3>(Optional) ZeroTrust Tenant Key:</h3>" +
                    "                </div>" +
                    "            </div>" +
                    "            <div class='user_input'>" +
                    "                <div class='text'>" +
                    "                    <input type='text' name='BEYOND_CORP_KEY' placeholder=''></input>" +
                    "                </div>" +
                    "            </div>" +
                    "            <div class='title'>" +
                    "                <div>" +
                    "                    <h3>(Optional) ZeroTrust Tenant Secret:</h3>" +
                    "                </div>" +
                    "            </div>" +
                    "            <div class='user_input'>" +
                    "                <div class='text'>" +
                    "                    <input type='text' name='BEYOND_CORP_SECRET' placeholder=''></input>" +
                    "                </div>" +
                    "            </div>" +
                    "        </div>" +
                    "    </div>" +
