...
require.config({
    paths: {
        // $SPLUNK_HOME/etc/apps/SPLUNK_APP_NAME/appserver/static/javascript/views/setup_view_demisto.js
        SetupViewDemisto: "../app/" + app_name + "/javascript/views/setup_view_demisto",
    },
});
...
require([
    ...
    // Custom files
    "SetupViewDemisto",
], function(Backbone, jquery, SetupViewDemisto) {
    var demisto_setup_view = new SetupViewDemisto({
        // Sets the element that will be used for rendering
        el: jquery("#main_container"),
    });

    console.log('rendering setup view...')

    demisto_setup_view.render();
});
